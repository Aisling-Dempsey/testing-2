...
class PartyTestsDatabase(unittest.TestCase):
    ...
    def test_games(self):
        ...
        result = self.client.get('/games',
                                 follow_redirects=True)
        self.assertNotIn("Settlers of Catan", result.data)
        self.assertNotIn('123 Magic Unicorn Way', result.data)
        self.assertIn("Please RSVP", result.data)


class PartyTestsSession(unittest.TestCase):
    """Flask tests that test session."""

    def setUp(self):
        """Stuff to do before session test."""
